...
Usage:
    from coordinate_converter import dms_to_decimal, decimal_to_dms

    # Captain enters: 57° 30.5' N
    lat = dms_to_decimal(57, 30.5, 'N')  # Returns 57.5083

    # Display from database: 57.5083 -> "57° 30.5' N"
    display = decimal_to_dms(57.5083, is_latitude=True)
"""
...
def decimal_to_dms_string(decimal: float, is_latitude: bool) -> str:
    ...
    Examples:
        decimal_to_dms_string(57.5083, True)   -> "57° 30.5' N"
        decimal_to_dms_string(-152.2583, False) -> "152° 15.5' W"
    """
...
def parse_dms_string(dms_str: str) -> tuple[int, float, str] | None:
    """
    Parse a DMS string like "57° 30.5' N" into components.

    Args:
        dms_str: String in format "DD° MM.M' D" or "DD°MM.M'D"

    Returns:
        Tuple of (degrees, minutes, direction) or None if invalid
    """
    import re

    # Pattern matches: 57° 30.5' N or 57°30.5'N
    pattern = r"(\d+)°\s*(\d+(?:\.\d+)?)['\u2032]\s*([NSEW])"
    match = re.match(pattern, dms_str.strip().upper())
...
def validate_latitude_dms(degrees: int, minutes: float) -> tuple[bool, str | None]:
    """Validate latitude in DMS for Alaska waters."""
    if degrees < 50 or degrees > 72:
        return False, "Latitude degrees must be between 50 and 72 for Alaska"
    if minutes < 0 or minutes >= 60:
        return False, "Minutes must be between 0 and 59.99"
    return True, None


def validate_longitude_dms(degrees: int, minutes: float) -> tuple[bool, str | None]:
    """Validate longitude in DMS for Alaska waters."""
    if degrees < 130 or degrees > 180:
        return False, "Longitude degrees must be between 130 and 180 for Alaska"
    if minutes < 0 or minutes >= 60:
        return False, "Minutes must be between 0 and 59.99"
    return True, None
