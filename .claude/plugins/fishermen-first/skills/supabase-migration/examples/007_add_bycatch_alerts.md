# Example: Complex Table with Multiple RLS Policies

This migration creates the bycatch alerts system with:
- Main table with GPS coordinates
- Supporting tables (contacts, email log)
- Role-based RLS policies
- Partial indexes for common queries

## Source: `sql/migrations/007_add_bycatch_alerts.sql`
...
1. **Multiple related tables** in single migration
2. **Role-based RLS** with separate policies for vessel_owner vs manager
3. **GPS coordinate constraints** with Alaska-specific bounds
4. **Status workflow** with CHECK constraint
5. **Partial indexes** for common query patterns
6. **Seed data** to bootstrap from existing data
7. **Helper view** for aggregated queries
