"""
Manage Processors - Admin CRUD page for processors table.
"""
...
def show():
    ...
    with col2:
        if st.button(f"+ Add {ITEM_NAME.title()}", use_container_width=True, key="processors_add_btn"):
...
def fetch_data() -> pd.DataFrame:
    """Fetch all processors from the table."""
    try:
        response = supabase.table(TABLE_NAME).select("*").order("processor_name").execute()
        if response.data:
...
def fetch_record(record_id: str) -> dict | None:
    """Fetch a single processor by ID."""
    try:
...
def display_data_table(data: pd.DataFrame):
    ...
    display_columns = ["processor_name", "contact_info"]

    column_config = {
        "processor_name": st.column_config.TextColumn("Processor Name"),
...
def show_action_buttons():
    ...
    with col1:
        ...
        if st.button("Edit Selected", disabled=edit_disabled, use_container_width=True, key="processors_edit_btn"):
    ...
    with col2:
        ...
        if st.button("Delete Selected", disabled=delete_disabled, use_container_width=True, key="processors_delete_btn"):
...
def show_form(data: pd.DataFrame):
    ...
    with st.expander(title, expanded=True):
        with st.form(key="processors_form"):
            processor_name = st.text_input(
                "Processor Name *",
                value=existing.get("processor_name", ""),
                placeholder="Enter processor name",
                key="processors_name_input",
            )
...
def show_delete_dialog():
    ...
    def confirm_delete():
        ...
        st.caption("Note: Processors with associated harvests cannot be deleted.")

        col1, col2 = st.columns(2)
        with col1:
            if st.button("Yes, Delete", use_container_width=True, type="primary", key="processors_confirm_delete_btn"):
                success_count = 0
                ...
                if success_count == count:
                    ...
                    st.error("Could not delete. Processors may have associated harvests.")
        ...
        with col2:
            if st.button("Cancel", use_container_width=True, key="processors_cancel_delete_btn"):
                st.session_state[f"{TABLE_NAME}_show_delete_dialog"] = False
                st.rerun()
...
def create_record(data: dict) -> bool:
    ...
    try:
        ...
        if "duplicate key" in error_msg.lower():
            st.error("A processor with this name already exists.")
        else:
...
def delete_record(record_id: str) -> bool:
    ...
    try:
        ...
        if "foreign key" in error_msg.lower():
            st.error("Cannot delete: this processor has associated harvests.")
        else:
