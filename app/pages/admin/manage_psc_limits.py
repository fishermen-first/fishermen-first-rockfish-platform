"""
Manage PSC Limits - CRUD operations for psc_limits table.
"""
...
TABLE_NAME = "psc_limits"
PAGE_TITLE = "Manage PSC Limits"
ITEM_NAME = "PSC limit"
...
def show():
    ...
    with col2:
        if st.button("+ Add PSC Limit", key="psc_limits_add_btn", use_container_width=True):
            st.session_state.psc_limits_mode = "add"
            st.session_state.psc_limits_selected_id = None

    # Check for PSC species
    psc_species = fetch_psc_species()
    if not psc_species:
        st.warning("No PSC species found. Mark species as PSC in the Species tab first.")
        return
    ...
    if data.empty:
        st.info("No PSC limits found. Click 'Add PSC Limit' to create one.")
    else:
        display_data_table(data)
        show_action_buttons()

    # Show form if in add/edit mode
    if st.session_state.psc_limits_mode in ["add", "edit"]:
        show_form()

    # Delete confirmation dialog
    if st.session_state.psc_limits_show_delete_dialog:
        show_delete_dialog()
...
def init_session_state():
    ...
    defaults = {
        "psc_limits_mode": "view",
        "psc_limits_selected_id": None,
        "psc_limits_selected_ids": [],
        "psc_limits_show_delete_dialog": False,
    }
...
def reset_mode():
    ...
    st.session_state.psc_limits_mode = "view"
    st.session_state.psc_limits_selected_id = None
    st.session_state.psc_limits_selected_ids = []


def fetch_data() -> pd.DataFrame:
    """Fetch all PSC limits with joined data."""
    try:
        response = supabase.table(TABLE_NAME).select("*").execute()
        if not response.data:
            return pd.DataFrame()

        limits = pd.DataFrame(response.data)
        ...
        # Join data
        limits["season_year"] = limits["season_id"].map(seasons).fillna("Unknown")
        limits["cooperative_name"] = limits["cooperative_id"].map(cooperatives).fillna("Unknown")
        limits["species_name"] = limits["species_id"].map(species).fillna("Unknown")
...
def fetch_psc_species() -> list[dict]:
    """Fetch only PSC species (where is_psc = true) for dropdown."""
    try:
        response = supabase.table("species").select("id, species_name, species_code").eq("is_psc", True).order("species_name").execute()
        return response.data or []
    except Exception:
        return []
...
def display_data_table(data: pd.DataFrame):
    ...
    display_columns = ["season_year", "cooperative_name", "species_name", "limit_amount"]
    ...
    column_config = {
        ...
        "species_name": st.column_config.TextColumn("PSC Species"),
        "limit_amount": st.column_config.NumberColumn("Limit (lbs)", format="%.0f"),
    }
...
def show_action_buttons():
    ...
    selected_ids = st.session_state.psc_limits_selected_ids

    col1, col2, col3 = st.columns([1, 1, 4])

    with col1:
        edit_disabled = len(selected_ids) != 1
        if st.button("Edit", key="psc_limits_edit_btn", disabled=edit_disabled, use_container_width=True):
            st.session_state.psc_limits_mode = "edit"
            st.session_state.psc_limits_selected_id = selected_ids[0]
            st.rerun()

    with col2:
        delete_disabled = len(selected_ids) == 0
        if st.button("Delete", key="psc_limits_delete_btn", disabled=delete_disabled, use_container_width=True):
            st.session_state.psc_limits_show_delete_dialog = True
            st.rerun()


def show_form():
    """Display add/edit form in an expander."""
    mode = st.session_state.psc_limits_mode
    is_edit = mode == "edit"

    existing = {}
    if is_edit:
        existing = fetch_record(st.session_state.psc_limits_selected_id) or {}

    title = "Edit PSC Limit" if is_edit else "Add New PSC Limit"
        ...
        with st.form("psc_limits_form"):
            ...
            with col1:
                ...
                selected_coop_id = st.selectbox(
                    ...
                    key="psc_limits_form_coop",
                )

            with col2:
                # PSC Species dropdown (filtered to is_psc=true)
                species_options = {s["id"]: f"{s['species_name']} ({s['species_code']})" for s in psc_species}
                ...
                limit_amount = st.number_input(
                    "Limit Amount (lbs) *",
                    min_value=0.0,
                    value=float(existing.get("limit_amount", 0)),
                    step=100.0,
                    format="%.0f",
                    key="psc_limits_form_amount",
                )
            ...
            if submitted:
                if limit_amount <= 0:
                    st.error("Limit Amount must be greater than 0.")
                    return
                ...
                if is_edit:
                    success = update_record(st.session_state.psc_limits_selected_id, record_data)
                else:
                    success = create_record(record_data)

                if success:
                    st.success(f"PSC limit {'updated' if is_edit else 'created'} successfully!")
                    reset_mode()
                    st.rerun()
...
def show_delete_dialog():
    ...
    @st.dialog("Delete PSC Limit(s)")
    def confirm_delete():
        st.warning(f"Are you sure you want to delete {count} PSC limit(s)? This cannot be undone.")

        col1, col2 = st.columns(2)
        with col1:
            if st.button("Yes, Delete", key="psc_limits_confirm_delete", use_container_width=True, type="primary"):
                success_count = 0
                ...
                if success_count == count:
                    st.success(f"Deleted {count} PSC limit(s).")
                else:
                    st.warning(f"Deleted {success_count} of {count} PSC limit(s).")

                st.session_state.psc_limits_show_delete_dialog = False
                reset_mode()
                st.rerun()

        with col2:
            if st.button("Cancel", key="psc_limits_cancel_delete", use_container_width=True):
                st.session_state.psc_limits_show_delete_dialog = False
                st.rerun()
...
def create_record(data: dict) -> bool:
    ...
    try:
        ...
        st.error(f"Error creating PSC limit: {e}")
        return False
...
def update_record(record_id: str, data: dict) -> bool:
    ...
    try:
        ...
        st.error(f"Error updating PSC limit: {e}")
        return False
...
def delete_record(record_id: str) -> bool:
    ...
    try:
        ...
        st.error(f"Error deleting PSC limit: {e}")
        return False
