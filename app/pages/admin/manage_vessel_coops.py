"""
Manage Vessel-Cooperative Assignments - Admin CRUD page for vessel_cooperative_assignments table.
...
TABLE_NAME = "vessel_cooperative_assignments"
PAGE_TITLE = "Vessel-Cooperative Assignments"
...
def show():
    ...
    with col2:
        if st.button(f"+ Add {ITEM_NAME.title()}", use_container_width=True, key="vessel_coops_add_btn"):
...
def fetch_data() -> pd.DataFrame:
    """Fetch all assignments with vessel and cooperative names."""
    try:
        ...
        # Get vessel names
        vessel_response = supabase.table("vessels").select("id, vessel_name").execute()
        vessel_names = {}
        if vessel_response.data:
            vessel_names = {v["id"]: v["vessel_name"] for v in vessel_response.data}
        ...
        # Add display names
        assignments["vessel_name"] = assignments["vessel_id"].map(vessel_names)
...
def display_data_table(data: pd.DataFrame):
    ...
    show_historical = st.toggle(
        ...
        key="vessel_coops_show_historical_toggle",
    )
    ...
    display_columns = ["vessel_name", "cooperative_name", "effective_from", "effective_to", "status"]
...
def show_action_buttons():
    ...
    with col1:
        ...
        if st.button("Edit Selected", disabled=edit_disabled, use_container_width=True, key="vessel_coops_edit_btn"):
    ...
    with col2:
        ...
        if st.button("End Assignment", disabled=end_disabled, use_container_width=True, key="vessel_coops_end_btn"):
...
def show_form():
    ...
    if not vessels:
        st.warning("No vessels found. Please add vessels first.")
        return
    if not coops:
        st.warning("No cooperatives found. Please add cooperatives first.")
        return

    vessel_options = {v["id"]: v["vessel_name"] for v in vessels}
    ...
    with st.expander(title, expanded=True):
        with st.form(key="vessel_coops_form"):
            # Vessel dropdown
            existing_vessel_id = existing.get("vessel_id")
            vessel_default_index = 0
            if existing_vessel_id and existing_vessel_id in vessel_ids:
                vessel_default_index = vessel_ids.index(existing_vessel_id)

            selected_vessel_id = st.selectbox(
                "Vessel *",
                options=vessel_ids,
                index=vessel_default_index,
                format_func=lambda x: vessel_options.get(x, "Unknown"),
                key="vessel_coops_vessel_select",
            )
...
def show_end_dialog():
    ...
    def confirm_end():
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Yes, End Assignment", use_container_width=True, type="primary", key="vessel_coops_confirm_end_btn"):
                if selected_ids:
        ...
        with col2:
            if st.button("Cancel", use_container_width=True, key="vessel_coops_cancel_end_btn"):
                st.session_state[f"{TABLE_NAME}_show_end_dialog"] = False
                st.rerun()
...
def create_record(data: dict) -> bool:
    ...
    try:
        ...
        if "duplicate key" in error_msg.lower():
            st.error("This vessel-cooperative assignment already exists.")
        else:
